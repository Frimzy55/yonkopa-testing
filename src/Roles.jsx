...
// Get all descendant permission IDs for a given menu item (by its permissionId)
const getAllDescendantIds = (permissionId) => {
  ...
  const collect = (currentItem, currentPath) => {
    ...
    // Nested menus (direct under this item)
...
// Get immediate parent permission ID for a given permission ID
const getParentPermissionId = (permissionId) => {
  ...
  if (path.length === 1) {
    // Parent is main menu
    return getPermissionId(path[0]);
  } else if (path.length === 2) {
    // Parent is submenu
    return getPermissionId(path[0], path[1]);
  }
  return null;
};
...
const Roles = () => {
  ...
  // ======================== Render functions (unchanged, but they now call the fixed handler) ========================
