// teller/ReverseDeposit.jsx
import React, { useState } from 'react';
import axios from 'axios';

const ReverseDeposit = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.type === 'deposit') {
  ...
  const handleReverse = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`/api/teller/transactions/${transactionId}/reverse`, 
        { reason, reversedBy: 'Admin User' },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      
      setSuccess('Deposit reversed successfully!');
      setReason('');
      setTimeout(() => {
        setSuccess('');
        setTransaction(null);
      ...
      setError(err.response?.data?.message || 'Failed to reverse deposit');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="reverse-deposit-container">
      <h4 className="mb-4">Reverse Deposit</h4>

      <div className="card">
        <div className="card-body">
          <h6 className="card-title mb-3">Search Deposit Transaction</h6>
          ...
          <div className="card-body">
            ...
            <div className="row mb-3">
              ...
              <div className="col-md-4">
                ...
                <p className="fw-bold text-success">GHS {transaction.amount?.toLocaleString()}</p>
              </div>
              <div className="col-md-4">
            ...
            <form onSubmit={handleReverse}>
              <div className="mb-3">
                ...
                <textarea
                  ...
                  placeholder="Please provide a detailed reason for reversing this deposit"
                  required
                />
              </div>
              <button type="submit" className="btn btn-danger" disabled={loading}>
                {loading ? 'Processing...' : 'Reverse Deposit'}
              </button>
            </form>
          </div>
        </div>
